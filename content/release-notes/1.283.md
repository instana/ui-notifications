...
## Features
### Dropwizard sensor
Instana now supports monitoring of all versions of Dropwizard 4.0 and earlier. You can now filter the Dropwizard custom metrics by using a regular expression. For more information, see [Monitoring Dropwizard](https://www.ibm.com/docs/en/instana-observability/saas?topic=technologies-monitoring-dropwizard).

### Limited access RBAC for Automation framework
You can select **Limited access** permission scope for **Automation** product area when you create a new user group. For more information, see [Manage user access](https://www.ibm.com/docs/en/instana-observability/saas?topic=instana-managing-user-access).
...
You can now add new service level objectives (SLO) widgets to display and analyze the performance of their services over time on their custom dashboards. For more information, see [Service level objectives widgets (public preview)](https://www.ibm.com/docs/en/instana-observability/saas?topic=instana-service-level-objectives-widgets-public-preview).

### Notes and Activity sidebar (public preview)
You can now add comments to the incidents by using the **Notes and Activity** sidebar. For more information, see [Adding notes to an incident (public preview)](https://www.ibm.com/docs/en/instana-observability/saas?topic=references-analyzing-incidents).
...
The Slack alert channel configuration now includes a switch **Display emojis**. With this switch, you can control whether Slack messages display emojis. For more information, see the [Slack alert channel](https://www.ibm.com/docs/en/instana-observability/saas?topic=apis-slack).
...
It is now possible to test the configuration for your DBmarlin integration within settings. For more information, see [DBmarlin documentation](https://www.ibm.com/docs/en/instana-observability/saas?topic=apis-dbmarlin).
