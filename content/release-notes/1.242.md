...
Instana now automatically discovers and monitors AWS API Gateway. For more information, see [AWS support](https://www.ibm.com/docs/en/instana-observability/saas?topic=agents-monitoring-amazon-web-services-aws).

### Open Beta

#### Analyze Infrastructure API

[Analyze Infrastructure](https://www.ibm.com/docs/en/instana-observability/saas?topic=analytics-analyze-infrastructure-open-beta) is now supported on the API.
...
For more information see [Synthetic Monitoring Open Beta](https://www.ibm.com/docs/en/instana-observability/saas?topic=instana-synthetic-monitoring-open-beta).
...
A new [Permission Scope](https://www.ibm.com/docs/en/instana-observability/saas?topic=instana-managing-user-access#permission-scope) for [Analyze Infrastructure](https://www.ibm.com/docs/en/instana-observability/saas?topic=analytics-analyze-infrastructure-open-beta) is added, which allows using the functionality for groups with limited scope if granted. By default, permissions are not granted to groups with currently limited scope.
...
The PoP helm charts are published. You can specify `--repo https://syntheticpops.instana.io/helm` when you run the helm installation command. For more information, see [Synthetic PoP Deployment](https://www.ibm.com/docs/en/instana-observability/saas?topic=beta-pop-deployment).
